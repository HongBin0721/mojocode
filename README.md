...
| `shift+tab` | 循环切权限档位：`read-only` → `ask` → `auto` → `full-access` → `plan` → `read-only`（预设会写进本工作区配置，`plan` 不落盘） |
| `鼠标点击` | 点底栏的权限档位弹出档位选项框（四个预设 + `plan` 全列出，键盘同样可用，`esc` 取消；与 `shift+tab` 同一份量，同样落盘）；授权确认框、回退选择器、`/setting` 面板的列表行点一下即选中。按下与抬起要落在同一格才算点击，所以拖选复制不会误触发 |
...
`plan` 一并列出、标出当前档，点或回车选定、`esc` 取消——与 `shift+tab` 同一份量，差别
只是由你指定落在哪一档，而不是盲切下一档。

切档位会写进**本工作区**的 `.mojocode/config.json`（与 `/approvals` 同一条落盘路径，
不碰全局配置：放宽是对某个工作区的信任声明，不该泄漏到别的目录），所以选一次就管到
下次在这个目录启动。`plan` 不落盘——它是一次协作方式的选择，不是档位。
