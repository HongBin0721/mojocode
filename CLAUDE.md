...
- `src/tools/` — builtin tools: `read write edit glob grep bash web_fetch todo` plus `web_search` when a search backend resolves. `summarizeToolResult` produces the localized one-line UI summary; tool *results* fed back to the model stay English. Web backend adapters live in `web-backends.ts`; both web tools funnel through `gate.checkNet` (per-domain confirm + remember).
