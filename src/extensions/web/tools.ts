...
import { runSearch } from './backends.js';
import { MAX_OUTPUT_CHARS, truncate } from '../../tools/context.js';

/**
 * 工具要的东西。不收 `ToolContext`:web 工具是扩展。后端是**惰性 getter**
 * ——config 会被就地改,现取现算才拿到当下的值。
 */
 ...
 * 解释);web_fetch 零依赖恒注册。没有授权、没有域名围栏(Pi 式)。
 */
export function createWebTools(ctx: WebToolDeps) {
  // 注册时机一次性求值:后端与 provider 工具同生命周期(/new 重建时再取)。
  const searchAvailable = ctx.searchBackend() !== undefined;
  return {
    web_fetch: createFetchTool(),
    ...(searchAvailable ? { web_search: createSearchTool(ctx) } : {}),
  };
}

function createSearchTool(ctx: WebToolDeps) {
  return tool({
    description:
