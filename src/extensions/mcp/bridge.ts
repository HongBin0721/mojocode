 ...
 *
 * 有意手写而不用 AI SDK 的实验性 MCP client:AI SDK 升级不会悄悄改变 MCP
 * 工具的行为,结果的压平与截断也由我们自己定。
 */
export interface BridgedMcpTool {
  /** 加了命名空间的工具名(`mcp__<server>__<tool>`)。 */
  name: string;
  create: () => Tool;
}

export function bridgeMcpTools(connections: McpConnection[]): BridgedMcpTool[] {
    ...
    for (const mcpTool of connection.tools) {
      ...
      bridged.push({
        ...
        create: () =>
          tool({
            description:
              mcpTool.description ?? `Tool "${mcpTool.name}" from MCP server "${connection.name}".`,
