...
interface Props {
   ...
   * 期间 App 未必有任何信号变化(两轮之间只有 bus 事件,中间那几十秒里
   * 没有状态更新)。这个组件自己按秒 tick,每次现读。
   */
 ...
 */
export function GoalLine(props: Props): JSX.Element {
    ...
    <Show when={label() !== undefined}>
      <Box justifyContent="flex-end" paddingRight={WIDTH_SAFETY}>
        ...
        <Text color={theme.dim}>
          {truncateWidth(`${glyphs.goal} ${label()!}`, Math.max(8, props.columns - WIDTH_SAFETY))}
        </Text>
      </Box>
    </Show>
  );
